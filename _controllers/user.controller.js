        ...
        User.findOne({ email: fields.email })
            .then(userFound => {
                if (userFound) {
                    ...
                    bcryptjs.hash(fields.password[0], 10)
                        .then(hashed => {
...
export const login = (req, res) => {
    ...
    form.parse(req, (error, fields, files) => {
        User.findOne({ email: fields.email })
            .then(userFound => {
                if (userFound !== null) {
                    bcryptjs.compare(fields.password[0], userFound.password)
                        .then(compared => {
                            if (compared) {
...
export const changePassword = (req, res) => {
    User.findOne({ _id: new ObjectId(fields.userId) })
        .then(found => {
            if (found !== null) {
                bcryptjs.compare(fields.oldPassword, found.password)
                    .then(compared => {
                        if (compared) {
                            bcryptjs.hash(fields.password, 10)
                                .then(hashed => {
                                    User.updateOne({ _id: new ObjectId(found._id) }, { $set: { password: hashed } })
                                        .then(updated => {
